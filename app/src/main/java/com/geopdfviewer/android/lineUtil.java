...
public class lineUtil {
    ...
    public static double[] getLineNormalEquation(float x1, float y1, float x2, float y2){
        if (x1 == x2 & y1 != y2){
    ...
    public static List<double[]> getParallelLineEquation(double[] line, double delta){
        if (line != null) {
            ...
            if ((line[0] > 0 & line[1] > 0) || (line[0] < 0 & line[1] < 0) || (line[0] != 0 & line[1] == 0)) {
    ...
    public static double[] getExactVerticalLine(List<double[]> lines, double[] mline, PointF p1, PointF p2){
        for (int i = 0; i < lines.size(); i++){
            ...
            if (pt.x >= ( p1.x >= p2.x ? p2.x : p1.x) & pt.x <= ( p1.x >= p2.x ? p1.x : p2.x) & pt.y >= ( p1.y >= p2.y ? p2.y : p1.y) & pt.y <= ( p1.y >= p2.y ? p1.y : p2.y)) continue;
    ...
    public static String getExternalPolygon(String line, double delta){
        ...
        for (int i = 0; i < size; i++){
            ...
            pointFS1.add(new PointF(Float.valueOf(pt[1]) * 10000, Float.valueOf(pt[0]) * 10000));
        }
        ...
        for (int i = 1; i < pointFS.size(); i = i + 2){
            ...
            line = line + pointFS.get(i).y / 10000 + "," + pointFS.get(i).x / 10000 + " ";
        }
